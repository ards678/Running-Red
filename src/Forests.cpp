...
namespace S{
    Forests::Forests(GameDataRef data): _data(data){
        ...
        sprite.setPosition(0,_data->window.getSize().y-sprite.getGlobalBounds().height-160);
        sprite2.setPosition(sprite.getGlobalBounds().width,_data->window.getSize().y-sprite2.getGlobalBounds().height-160);

        _ForestSprite.push_back(sprite);
        _ForestSprite.push_back(sprite2);
    }

    void Forests::MoveForests(float speed, float dt){
